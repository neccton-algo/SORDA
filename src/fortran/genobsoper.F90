...
program genobsoper
 ...
 do ntime=startntime,endntime
   ...
   call usave_Real2D(trim(path)//str,Hop,9999.)

!#ifdef .true.
   call packSparseMatrix(Hindex,Hcoeff,ObsML,ModML,H)
   call usave_Real1D(trim(path)//trim(str)//'.i',real(H%i(1:H%nz)),9999.)
   call usave_Real1D(trim(path)//trim(str)//'.j',real(H%j(1:H%nz)),9999.)
   call usave_Real1D(trim(path)//trim(str)//'.s',H%s(1:H%nz),9999.)
!#endif
